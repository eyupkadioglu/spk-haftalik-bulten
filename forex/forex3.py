...
CONTACT_KEYWORDS = [
    ...
    "support", "help", "yardim", "customer-service",
    "customer-support", "reach-us", "about", "hakkimizda"
]
...
AUTH_KEYWORDS = [
    ...
    "portal", "uye", "uye-ol", "kayit",
    "hesap", "hesap-olustur", "giris",
    "oturum", "b2b", "dealer-login", "bayi", "musteri"
]
...
def fetch_once(url: str, user_agent=None):
    result = {
        "requested_url": url,
        "final_url": url,
        "status_code": None,
...
def fetch_url(url: str):
    ...
    # 2) Ikinci deneme: farkli User-Agent ile tekrar
    if result["error"]:
        time.sleep(random.uniform(1.5, 3.0))
        result2 = fetch_once(url, user_agent=random.choice(USER_AGENTS))
        if result2["html"]:
            return result2

        # 3) HTTPS ise ve hic HTML gelmediyse son care HTTP fallback
        parsed = urlparse(url)
        if parsed.scheme == "https":
            ...
            time.sleep(random.uniform(1.5, 3.0))
            result3 = fetch_once(http_url, user_agent=random.choice(USER_AGENTS))
            if result3["html"]:
                result3["error"] = (result["error"] + " | HTTPS basarisizdi, HTTP fallback basarili")
                return result3
...
<body>
    ...
    <div class="note">
        ...
        - Yine de tamamen korumali sitelerde requests yetersiz kalabilir; o durumda Playwright gerekir.
    </div>
</body>
</html>
"""
        ...
        try:
            ...
            for idx, url in enumerate(urls, start=1):
                ...
                try:
                    ...
                    self.root.after(0, self.log, f"Hata: {url} -> {clean_exception_message(e)}")
                    results.append({
                        "site": url,
                        "final_url": url,
                        "status_code": "",
                        ...
                        "lang_links": [],
                        "scanned_pages": [],
                        "error": clean_exception_message(e),
                    })
