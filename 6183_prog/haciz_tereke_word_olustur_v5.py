...
def belge_turu_sec(root: tk.Tk) -> Optional[str]:
    ...
    btn_takasbank.grid(row=1, column=0, columnspan=2, padx=8, pady=6)
...
def tcknleri_al(root: tk.Tk) -> Optional[List[str]]:
    while True:
        ...
        mesajlar = []
        if not tcknler:
            mesajlar.append("En az bir geçerli 10 haneli VKN veya 11 haneli TCKN giriniz.")
        if hatali:
...
def main() -> None:
    ...
    try:
        ...
        if belge_islem == "TAKASBANK_BILDIRIM":
            ...
            for tckn in tcknler:
                try:
                    takasbank_verileri.append(excelden_6183_kaydi_bul(excel_yolu, tckn))
