...
def kisi_ceza_ifadesi_olustur(veri: Dict[str, Any]) -> str:
    gercek_sayisi, tuzel_sayisi = kisi_turu_sayilari(veri)
    toplam = gercek_sayisi + tuzel_sayisi

    if gercek_sayisi and tuzel_sayisi:
        kisi_ifadesi = "gerçek ve tüzel kişi" if toplam == 1 else "gerçek ve tüzel kişiler"
    elif gercek_sayisi:
...
def tablo_satiri_yaz(row, veri: Dict[str, str]) -> None:
    ...
    hucre_yaz(row.cells[5], veri["tahsilat_tarihi_fis_no"])
...
def excelden_vergi_dairesi_kaydi_bul_girdi(excel_yolu: Path, girdi: str) -> Dict[str, str]:
    temiz_girdi = str(girdi or "").strip()
    rakamlar = re.sub(r"\D", "", temiz_girdi)
    if rakamlar and len(rakamlar) in (10, 11):
        return excelden_vergi_dairesi_kaydi_bul(excel_yolu, normalize_tckn(temiz_girdi))
    ...
    ws = wb[VERGI_DAIRESI_SHEET_ADI]
    aranan = arama_metni_normalize(temiz_girdi)
    ...
    for row in range(2, ws.max_row + 1):
        ...
        ths_fis_no = hucre_metni(ws.cell(row=row, column=15))
        odeme_tutari = hucre_metni(ws.cell(row=row, column=17))
        kimlik_no_alan = vergi_dairesi_id_ayikla(raw_kisi) or rakamlar
...
def kalan_alacak_ipc_tutari_bul(kalan_alacak_indeksi: Dict[str, Any], girdi: str, veri: Dict[str, str]) -> Tuple[Optional[str], str]:
    arama_girdisi = str(girdi or "").strip()
    rakamlar = re.sub(r"\D", "", arama_girdisi)
    tckn = normalize_tckn(arama_girdisi) if rakamlar and len(rakamlar) == 11 else ""
    aranan_unvan = arama_metni_normalize(arama_girdisi if not tckn else veri.get("muhatap", ""))
...
def kisi_ipc_kayitlari(kalan_alacak_indeksi: Dict[str, Any], girdi: str, veri: Dict[str, str]) -> List[Dict[str, Any]]:
    rakamlar = re.sub(r"\D", "", str(girdi or ""))
    tckn = normalize_tckn(girdi) if rakamlar and len(rakamlar) == 11 else ""
    aranan_unvan = arama_metni_normalize(str(girdi or "") if not tckn else veri.get("muhatap", ""))
...
def tcknleri_al(root) -> Optional[List[str]]:
    ...
    etiket = tk.Label(
        pencere,
        text="Gerçek kişi için TCKN, tüzel kişi için ünvan giriniz (her satıra bir tane):",
        font=("Segoe UI", 11),
        padx=24,
        pady=18,
    )
    ...
    metin_alani = tk.Text(pencere, width=40, height=10, font=("Consolas", 10))
