...
def turkce_baslik_duzeni(metin: str) -> str:
    """Her kelimenin ilk harfini büyük, kalanını küçük yapar."""
    def kelime_duzenle(eslesme) -> str:
        kelime = eslesme.group(0)
        if kelime.isdigit():
            return kelime
        kucuk = turkce_kucuk_harf(kelime)
        return turkce_buyuk_harf(kucuk[0]) + kucuk[1:]

    metin = str(metin or "").strip()
    metin = re.sub(r"\s+", " ", metin)
    metin = re.sub(r"\s+([.,;:])", r"\1", metin)
    metin = re.sub(r"([.,;:])(?=\S)", r"\1 ", metin)
    return re.sub(r"[A-Za-zÇĞİÖŞÜçğıöşü]+", kelime_duzenle, metin).strip()


def mahkeme_adi_duzenle(mahkeme: str) -> str:
    return turkce_baslik_duzeni(mahkeme)


def firma_unvani_duzenle(unvan: str) -> str:
    """Firma unvanını başlık düzenine çevirir; bağlaçları küçük bırakır."""
    metin = turkce_baslik_duzeni(unvan)
    metin = re.sub(r"\bVe\b", "ve", metin)
    metin = re.sub(r"\bİle\b", "ile", metin)
    metin = re.sub(r"\bA\.\s*Ş\.", "A.Ş.", metin)
    return metin


def kisi_adi_duzenle(ad_soyad: str) -> str:
    """Kişi adında adları başlık düzeninde, soyadı büyük harfle yazar."""
    parcalar = str(ad_soyad or "").strip().split()
    if not parcalar:
        return ""
    if len(parcalar) == 1:
        return turkce_baslik_duzeni(parcalar[0])

    adlar = " ".join(turkce_baslik_duzeni(parca) for parca in parcalar[:-1])
    soyad = turkce_buyuk_harf(turkce_kucuk_harf(parcalar[-1]))
    return f"{adlar} {soyad}".strip()


def ad_unvan_duzenle(metin: str, kimlik_turu: str = "", kisi_turu: str = "") -> str:
    """TCKN/gerçek kişi için kişi adı, VKN/tüzel kişi için firma unvanı düzeni uygular."""
    kimlik_turu_norm = turkce_buyuk_harf(kimlik_turu)
    kisi_turu_norm = turkce_buyuk_harf(kisi_turu)
    if kimlik_turu_norm == "VKN" or "TÜZEL" in kisi_turu_norm:
        return firma_unvani_duzenle(metin)
    return kisi_adi_duzenle(metin)


def vergi_dairesi_id_ayikla(metin: str) -> str:
    metin = str(metin or "").strip()
    eslesme = re.search(r"\((\d{10,11})\)", metin)
    if eslesme:
...
def vergi_dairesi_muhatap_ayikla(metin: str) -> str:
    metin = str(metin or "").strip()
    metin = re.sub(r"\s*-\s*\(\d{10,11}\)", "", metin).strip()
    return metin


def vergi_dairesi_paragraf_yaz(paragraph, text: str) -> None:
    fmt = paragraf_ilk_run_format(paragraph)
    paragraf_yaz(paragraph, text, fmt=fmt, sariyi_temizle=FINALDE_SARI_ALANLARI_TEMIZLE)


def turkce_ilgi_eki(metin: str) -> str:
    """Özel ada getirilecek ilgi eki: Ankara'nın, Mahkemesi'nin, Kurulu'nun."""
    metin = str(metin or "").strip()
    unluler = "aıoueiöüAIOUEİÖÜ"
    son_unlu = ""
    ...
    if not son_unlu:
        ...
        son_unlu = turkce_kucuk_harf(son_unlu)
        if son_unlu in "aı":
            ek = "ın"
        elif son_unlu in "ou":
            ek = "un"
    ...
    for karakter in reversed(metin):
        if karakter.isalpha():
            son_harf = karakter
            break
    if turkce_kucuk_harf(son_harf) in "aıoueiöü":
        ek = "n" + ek
    return "'" + ek
...
def belge_turu_sec(root: tk.Tk) -> Optional[str]:
    ...
    btn_vergi = tk.Button(
        buton_frame,
        text="Vergi Dairesi Yazısı",
        width=24,
        height=2,
        command=lambda: sec("VERGI_DAIRESI"),
    )
    btn_vergi.grid(row=1, column=1, padx=8, pady=6)
    ...
    btn_ytm_ici.grid(row=2, column=0, padx=8, pady=6)
...
def vergi_dairesi_turu_sec(root: tk.Tk) -> Optional[str]:
    ...
    pencere = tk.Toplevel(root)
    pencere.title("Vergi Dairesi Yazısı Türü")
    pencere.resizable(False, False)
    pencere.grab_set()

    etiket = tk.Label(
        pencere,
        text="Hangi Vergi Dairesi yazısını oluşturmak istersiniz?",
        font=("Segoe UI", 11),
        padx=24,
        pady=18,
    )
...
def excelden_6183_kaydi_bul(excel_yolu: Path, kimlik_no: str) -> Dict[str, str]:
    ...
    ad_soyad_ham = hucre_metni(ws.cell(row=row, column=2))      # B kolonu
    kimlik_no = normalize_tckn_vkn(ws.cell(row=row, column=5).value)
    kisi_turu = hucre_metni(ws.cell(row=row, column=4))         # D kolonu
    ad_soyad = ad_unvan_duzenle(ad_soyad_ham, kimlik_turu_belirle(kimlik_no), kisi_turu)
...
def excelden_tereke_kaydi_bul(excel_yolu: Path, kimlik_no: str) -> Dict[str, str]:
    ...
    muris_ad_soyad = ad_unvan_duzenle(
        hucre_metni(ws.cell(row=row, column=2)),  # B kolonu
        kimlik_turu=kimlik_turu,
    )
    mahkeme = mahkeme_adi_duzenle(hucre_metni(ws.cell(row=row, column=4)))  # D kolonu
...
def excelden_vergi_dairesi_kaydi_bul(excel_yolu: Path, kimlik_no: str) -> Dict[str, str]:
    wb = load_workbook(excel_yolu, data_only=True)
    ...
    for row in range(2, ws.max_row + 1):
        ...
        vd_kodu = normalize_tckn_vkn(ws.cell(row=row, column=12).value)

        if kimlik_no_alan == kimlik_no or vd_kodu == kimlik_no:
            eslesen_satirlar.append(row)
    ...
    pc_tutari = hucre_metni(ws.cell(row=row, column=17))
    tahsilat_tarihi = hucre_metni(ws.cell(row=row, column=16), tarih_mi=True)
    tahsilat_fis_no = hucre_metni(ws.cell(row=row, column=15))
    tahsilat_tarihi_fis_no = ""
    if tahsilat_tarihi and tahsilat_fis_no:
        tahsilat_tarihi_fis_no = f"{tahsilat_tarihi} - {tahsilat_fis_no}"
    elif tahsilat_tarihi:
        tahsilat_tarihi_fis_no = tahsilat_tarihi
    elif tahsilat_fis_no:
        tahsilat_tarihi_fis_no = tahsilat_fis_no

    eksikler = []
    if not muhatap:
        eksikler.append("C / İlgili Kişi/Şirket")
    if not vergi_dairesi_adi:
        eksikler.append("M / Vergi Dairesi Adı")
    if not karar_tarihi or not karar_no:
        eksikler.append("D/E / Karar Tarihi ve Sayısı")
    if not pc_tutari:
        eksikler.append("R / Tutar(TL)")
    if not tahsilat_tarihi_fis_no:
        eksikler.append("P / Tahsilat Tarihi ve Fiş No")
    if eksikler:
        raise ValueError(
            f"TCKN/VKN bulundu fakat {row}. satırda eksik veri var: " + ", ".join(eksikler)
        )
    ...
    return {
        ...
        "karar_no": karar_no,
        "pc_tutari": pc_tutari,
        "odenen_tutar": pc_tutari,
        "tahsilat_tarihi_fis_no": tahsilat_tarihi_fis_no,
        "sayi": "",
        "tarih": date.today().strftime("%d.%m.%Y"),
        "konu": f"İPC- {muhatap} Hk.",
...
def hucrede_sari_var_mi(cell) -> bool:
    return any(paragrafta_sari_var_mi(p) for p in cell.paragraphs)
...
def belgedeki_paragraflar(doc: Document) -> List:
    paragraflar = list(doc.paragraphs)

    for table in doc.tables:
        for row in table.rows:
            for cell in row.cells:
                paragraflar.extend(cell.paragraphs)

    return paragraflar


def mahkeme_tereke_bakiyesiz_sabit_paragraf_mi(paragraph) -> bool:
    metin = paragraph.text.strip()
    return (
        "Merkezimiz nezdinde herhangi bir emanet" in metin
        and "rastlanılmamıştır" in metin
    )
...
def ilgi_alani_doldur(doc: Document, ilgi_tarihi: str, ilgi_sayisi: str) -> None:
    yeni_ilgi = f"İlgi: {ilgi_tarihi} tarih ve {ilgi_sayisi} sayılı yazınız."
    for p in doc.paragraphs:
        if p.text.strip().startswith("İlgi:"):
            fmt = paragraf_ilk_run_format(p)
...
def vergi_dairesi_word_olustur(sablon_yolu: Path, cikti_yolu: Path, veri: Dict[str, str]) -> None:
    ...
    paragraf_yaz(sarili_paragraflar[0], turkce_buyuk_harf(f"{veri['vergi_dairesi_adi']} Vergi Dairesi Müdürlüğü"), fmt=paragraf_ilk_run_format(sarili_paragraflar[0]), sariyi_temizle=FINALDE_SARI_ALANLARI_TEMIZLE)

    # Sayi / tarih alanları
    for p in belgedeki_paragraflar(doc):
        if "[SAYI]" in p.text or "[TARIH]" in p.text:
            yeni_metin = p.text.replace("[SAYI]", veri.get("sayi", "")).replace("[TARIH]", veri.get("tarih", ""))
    ...
    for table in doc.tables:
        for row in table.rows:
            ...
            if any(text == "Konu" for text in row_text):
                ...
                hucre_yaz(hedef_cell, veri["konu"])
                break
        else:
            continue
        break
    ...
    hucre_yaz(data_row.cells[0], veri["muhatap"])
    hucre_yaz(data_row.cells[1], veri["kimlik_no"])
    hucre_yaz(data_row.cells[2], f"{veri['karar_tarihi']}-{veri['karar_no']}")
    hucre_yaz(data_row.cells[3], veri["pc_tutari"])
    hucre_yaz(data_row.cells[4], veri["odenen_tutar"])
    hucre_yaz(data_row.cells[5], veri["tahsilat_tarihi_fis_no"])
...
def mahkeme_tereke_bakiyesiz_kisi_turu_duzenle(doc: Document, kimlik_turu: str) -> None:
    if kimlik_turu != "VKN":
        return

    for p in belgedeki_paragraflar(doc):
        if not mahkeme_tereke_bakiyesiz_sabit_paragraf_mi(p):
            continue

        for run in p.runs:
            run.text = run.text.replace("TCKN", "VKN").replace("gerçek", "tüzel")
        return
...
def mahkeme_tereke_word_olustur(
    ...
    ilgi = f"İlgi: {ilgi_tarihi} tarih ve {dosya_tur} sayılı yazınız."
    ...
    if not bakiyeli_mi:
        mahkeme_tereke_bakiyesiz_kisi_turu_duzenle(doc, kimlik_turu)
...
def takasbank_bildirim_satiri(veri: Dict[str, str]) -> str:
    ...
    return f"{haczi_koyan}{turkce_ilgi_eki(haczi_koyan)} {veri['ilgi_tarihi']} tarih ve {veri['ilgi_sayisi']} sayılı yazısı."
...
def ytm_ici_talep_ek_satiri(veri: Dict[str, str], sira_no: int) -> str:
    ...
    return (
        f"{sira_no}- {mahkeme}{turkce_ilgi_eki(mahkeme)} {veri['ilgi_tarihi']} tarih ve "
...
def ytm_ici_talep_konu_doldur(doc: Document, veriler: List[Dict[str, str]]) -> None:
    ...
    for table in doc.tables:
        for row in table.rows:
            if any(cell.text.strip().lower() == "konu" for cell in row.cells):
                for cell in row.cells:
                    if hucrede_sari_var_mi(cell):
                        hucre_yaz(cell, konu)
                        return

    ytm_table = ytm_ici_talep_tablosu_bul(doc)
    for table in doc.tables:
        if table is ytm_table:
            continue
        for row in table.rows:
            for cell in row.cells:
                if hucrede_sari_var_mi(cell):
...
def ytm_ici_talep_tablosu_doldur(doc: Document, veriler: List[Dict[str, str]]) -> None:
    ...
    ornek_satir_index = None
    for index, row in enumerate(table.rows[1:], start=1):
        if any(hucrede_sari_var_mi(cell) for cell in row.cells):
            ornek_satir_index = index
            break
    if ornek_satir_index is None:
        raise ValueError("YTM içi talep şablonunda sarı dinamik tablo satırı bulunamadı.")

    ornek_satir = deepcopy(table.rows[ornek_satir_index]._tr)
    for row in list(table.rows[ornek_satir_index:]):
...
def ytm_ici_talep_ekleri_doldur(doc: Document, veriler: List[Dict[str, str]]) -> None:
    ek_paragraflar = [
        ...
        if paragrafta_sari_var_mi(p)
        and (
            re.match(r"^\s*\d+\s*-", p.text.strip())
            or re.search(r"(^|\n)\s*EK\s*:\s*(\n\s*)?\d+\s*-", p.text.strip(), flags=re.IGNORECASE)
        )
    ]
    ...
    ilk_metin = ek_paragraflar[0].text.strip()
    if re.search(r"(^|\n)\s*EK\s*:", ilk_metin, flags=re.IGNORECASE):
        ek_metni = "EK:\n" + ek_metni
...
def main() -> None:
    ...
    try:
        ...
        elif secim == "VERGI_DAIRESI":
            alt_secim = vergi_dairesi_turu_sec(root)
            if alt_secim is None:
                return
            belge_islem = alt_secim
        ...
        elif belge_islem == "VERGI_DAIRESI_SPK":
        ...
        elif belge_islem in {"VERGI_DAIRESI_SPK", "VERGI_DAIRESI_VD"}:
            for tckn in tcknler:
                try:
                    veri = excelden_vergi_dairesi_kaydi_bul(excel_yolu, tckn)
                    coklu_satir_uyarisi_ekle(veri, "kimlik_no")
                    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                    dosya_adi = f"{temiz_dosya_adi(veri['muhatap'])}_{veri['kimlik_no']}_vergi_dairesi_{timestamp}.docx"
                    cikti_yolu = base_dir / CIKTI_KLASOR_ADI / dosya_adi
                    vergi_dairesi_word_olustur(sablon_yolu, cikti_yolu, veri)
                    olusan_dosyalar.append(cikti_yolu)
                    log_kaydi_ekle(
                        log_wb,
                        log_yolu,
